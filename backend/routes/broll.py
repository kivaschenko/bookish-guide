...
@router.post("/{broll_id}/process")
async def process_broll(
    broll_id: int,
    current_user: UserResponse = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Manually trigger processing for a B-roll video (generate thumbnail and AI analysis)."""
    # Verify user has access to this B-roll
    query = select(BRoll).where(
        and_(
            BRoll.id == broll_id,
            BRoll.uploaded_by == current_user.id,  # Only owner can process
        )
    )
    ...
    if not broll:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="B-roll item not found or access denied",
        )

    # Only process video files
    if not broll.mime_type.startswith("video/"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Processing is only available for video files",
        )

    try:
        logger.info(f"Processing B-roll ID {broll_id} for user {current_user.id}")

        broll_service = BRollService()

        # Set status to processing
        broll.status = BRollStatus.PROCESSING.value
        await db.commit()

        # Extract metadata
        metadata_result = await broll_service.extract_video_metadata(broll.filename)
        logger.info(
            f"Metadata extraction result for {broll.filename}: {metadata_result}"
        )
        ...
        if metadata_result and metadata_result.get("analysis"):
            # Extract AI description and tags from analysis
            analysis = metadata_result.get("analysis", "")
            ai_description = analysis  # Use full analysis as description
            ...
            if "Keywords:" in analysis:
                ...
                # Clean AI description to remove keywords section for cleaner display
                ai_description = analysis.split("Keywords:")[0].strip()

            # Update the B-roll with AI analysis
            broll.ai_description = ai_description
            broll.ai_tags = json.dumps(ai_tags) if ai_tags else None
            logger.info(
                f"AI analysis for B-roll {broll_id}: Description length={len(ai_description) if ai_description else 0}, Tags={ai_tags}"
            )
        broll.status = BRollStatus.AVAILABLE.value
        await db.commit()

        logger.info(f"Successfully processed B-roll {broll_id}")

        return {
            "message": "Video processing completed successfully",
            "ai_description": broll.ai_description,
            "ai_tags": ai_tags,
            "thumbnail_generated": True,  # Assuming thumbnail was generated by meta_extractor
        }

    except Exception as e:
        logger.error(f"Failed to process B-roll {broll_id}: {str(e)}")
