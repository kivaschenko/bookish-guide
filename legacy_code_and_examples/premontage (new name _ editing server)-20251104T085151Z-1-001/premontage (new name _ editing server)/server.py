...
def load_auth_config():
    ...
    current_dir = Path(__file__).parent.parent.parent  # src/premontage/server.py -> racine
...
class PremontageHandler(BaseHTTPRequestHandler):
    ...
    def _serve_timeline_data(self):
        ...
        if not self.broll_timing_file.exists():
            ...
            self._send_response(404, '{"error": "broll_timing.json not found"}', "application/json")
            return
        ...
        print(f"DEBUG SERVEUR: Réponse JSON préparée, taille: {len(response_data)} caractères")
    ...
    def _send_response(self, code, data, content_type):
        ...
        self.send_header('Content-type', content_type)
        self.send_header('Content-length', len(data.encode() if isinstance(data, str) else data))
        self.end_headers()
...
def start_premontage_server(config=None, port=47393):
    ...
    if not broll_timing_file.exists():
        ...
        print("   Exécutez d'abord: python exponential_video.py --vectorbroll NOM_PROJET")
        return False
    ...
    try:
        ...
        # Démarrer le serveur
        server = HTTPServer(('0.0.0.0', port), lambda *args, **kwargs: PremontageHandler(*args, config=config, **kwargs))
        ...
        try:
            import requests
            response = requests.get('http://leader-referencement.com/get_current_ip.php', timeout=5)
            if response.status_code == 200:
                data = response.json()
                public_ip = data.get('ip')
                if public_ip:
                    print(f"🌐 Accessible via: http://localhost:{port} (local) ou http://{public_ip}:{port} (externe)")
                else:
                    print(f"🌐 Accessible via: http://localhost:{port} (local) ou http://votre-ip:{port} (externe)")
            else:
                print(f"🌐 Accessible via: http://localhost:{port} (local) ou http://votre-ip:{port} (externe)")
        except Exception:
            print(f"🌐 Accessible via: http://localhost:{port} (local) ou http://votre-ip:{port} (externe)")
