...
def _generate_bullet_metadata(audio_files, config, temp_dir):
    ...
    for bullet_index, (audio_file, bullet_text) in enumerate(zip(audio_files, bullet_texts), 1):
    ...
    logging.info(f"⏱️ Total duration: {cumulative_time:.2f}s ({len(audio_files)} bullets)")
...
def generate_gemini_tts_audio_via_proxy(config, granularity):
    ...
    proxy_url = config.get('api', {}).get('gemini', {}).get('proxy_url', 'https://leader-referencement.com/gemini_tts.php')
    ...
    logging.info(f"🌐 Gemini TTS audio generation via PROXY for {len(text_files)} {prefix} files")
    logging.info(f"📡 Proxy used: {proxy_url}")
    logging.info(f"Rate limiting enabled: {MIN_INTERVAL_SECONDS} seconds between each API call")
    ...
    for i, text_file in enumerate(text_files):
        try:
            ...
            try:
                ...
                logging.error(f"Unable to convert '{file_number_str}' to number for {text_file}")
                continue
            ...
            if last_call_timestamp is not None:
                ...
                if time_since_last_call < MIN_INTERVAL_SECONDS:
                    ...
                    logging.info(f"⏳ Rate limiting : attente de {wait_time:.1f} secondes avant l'appel suivant...")
                    time.sleep(wait_time)
            
            # Generate audio with Gemini TTS via proxy
            logging.info(f"🌐 Audio generation via proxy for {prefix} {file_number}... ({i+1}/{len(text_files)})")
            ...
            if response.status_code != 200:
                ...
                raise Exception(f"Erreur proxy HTTP {response.status_code}: {error_detail}")
            ...
            try:
                ...
                logging.info(f"✅ JSON parsed successfully, keys: {list(result.keys())}")
            ...
            import subprocess
            convert_cmd = ["ffmpeg", "-y", "-i", str(temp_wav), "-q:a", "0", str(audio_path)]
    ...
    logging.info(f"🎵 Audio generation via proxy completed: {len(audio_files)} MP3 files created")
...
def generate_gemini_tts_audio_direct(config, granularity):
    ...
    try:
        ...
        logging.info(f"Gemini TTS audio generation for {len(text_files)} {prefix} files")
        logging.info(f"Rate limiting enabled: {MIN_INTERVAL_SECONDS} seconds between each API call")
        ...
        for i, text_file in enumerate(text_files):
            try:
                ...
                logging.info(f"Audio generation for {prefix} {file_number}... ({i+1}/{len(text_files)})")
                    ...
                    try:
                        ...
                        if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str:
                            if retry_count < max_retries - 1:
                                logging.warning(f"⚠️ Error 429 (quota exceeded) for {prefix} {file_number}, waiting {retry_delay}s before retry {retry_count + 1}/{max_retries}")
                                time.sleep(retry_delay)
                                continue
                            else:
                                logging.error(f"❌ Final failure after {max_retries} attempts for {prefix} {file_number}")
                                logging.error(f"❌ Gemini TTS quota exhausted permanently. Stopping script.")
                                logging.error(f"💡 Tip: Wait until tomorrow or upgrade to a paid Google Cloud plan")
                                sys.exit(1)
                        else:
                            # Autre erreur, on arrête les retries
                            logging.error(f"❌ Erreur non-429 pour {prefix} {file_number}: {e}")
                            raise
                ...
                logging.info(f"📡 API response received in {response_time:.1f}s, {len(data)} audio bytes")
                ...
                if temp_wav.exists():
                    ...
                    logging.info(f"🎵 Temporary WAV created: {temp_wav.name} ({wav_size} bytes)")
                else:
                    logging.error(f"❌ WAV creation failed: {temp_wav}")
                    continue
                
                # Convertir WAV vers MP3 avec ffmpeg (conversion rapide)
                audio_filename = f"{prefix}{file_number:02d}.mp3"  # Format: bullet01.mp3, rush01.mp3
                audio_path = audio_dir / audio_filename
                
                try:
                    # Utiliser ffmpeg pour la conversion (plus rapide que pydub)
                    import subprocess
                    logging.info(f"🔄 Conversion ffmpeg: {temp_wav.name} → {audio_filename}")
                    result = subprocess.run([
                        'ffmpeg', '-y',  # -y pour écraser sans demander
                        '-i', str(temp_wav),  # fichier d'entrée WAV
                        '-acodec', 'libmp3lame',  # codec MP3
                        '-ab', '128k',  # bitrate 128k (bon compromis qualité/taille)
                    ...
                    if result.returncode == 0:
                        ...
                        if audio_path.exists():
                            ...
                            logging.info(f"✅ MP3 created successfully: {audio_filename} ({mp3_size} bytes)")
                            # Delete temporary WAV file
                            temp_wav.unlink()
                            logging.info(f"🗑️ Temporary WAV deleted: {temp_wav.name}")
                        else:
                            logging.error(f"❌ MP3 not found after conversion: {audio_path}")
                            audio_path = temp_wav
                ...
                if audio_path.exists():
                    ...
                    logging.info(f"🎯 Final audio file confirmed: {audio_path.name} ({final_size} bytes)")
                else:
                    logging.error(f"💥 PROBLÈME CRITIQUE: Fichier audio final non trouvé: {audio_path}")
                    continue
                ...
                logging.error(f"Erreur lors de la génération audio pour {text_file}: {e}")
                continue
        ...
        if actual_count == expected_count:
            logging.info(f"✅ Génération terminée avec succès : {actual_count}/{expected_count} fichiers")
        else:
            missing_count = expected_count - actual_count
            logging.warning(f"⚠️ Génération incomplète : {missing_count} fichier(s) manquant(s)")
            
            # Identifier les fichiers manquants
            expected_numbers = {int(f.stem.split('_')[0].replace(prefix, '')) for f in text_files}
...
def parse_arguments():
    ...
    parser.add_argument("-s", "--script", action="store_true", help="Generate scripts only")
    parser.add_argument("-a", "--audio", action="store_true", help="Generate TTS audio only (OpenAI)")
    parser.add_argument("-ag", "--audio-gemini", action="store_true", help="Generate TTS audio only (Google Gemini)")
    parser.add_argument('--meta', action='store_true', help="Extract and analyze b-roll video metadata")
    parser.add_argument('--vectorbroll', action='store_true',
    ...
    parser.add_argument('--testbrollchoice', action='store_true',
                       help='Test b-roll selection for first rush only')
    parser.add_argument("--brolls-selection-correct", action="store_true",
    ...
    parser.add_argument("--storykw", action="store_true", help="Generate keywords list for script i3.txt illustration images")
    parser.add_argument("--youtube_help", action="store_true", help="Generate YouTube information (titles, description, tags) based on script i3.txt")
    parser.add_argument("project_name", nargs='?', help="Project name (required except for --meta)")
    parser.add_argument("--get_ori", metavar="YOUTUBE_URL", help="Get YouTube video transcript (original audio language) and save to ../projects/<project>/ori_vid.txt")
...
def validate_project_argument(args):
    ...
    while project_name.startswith('projects/') or project_name.startswith('../projects/'):
    ...
    if not re.match(r'^[a-zA-Z0-9_-]+$', project_name):
        ...
        logging.error("   → Utilisez uniquement des lettres, chiffres, tirets et underscores")
...
def ensure_project_directories(project_name):
    ...
    if not project_ori_vid_path.exists():
        ...
        logging.info(f"💡 Ajoutez votre transcript dans ce fichier avant de lancer la génération")
...
def play_completion_sound():
    ...
    for _ in range(5):
        print("\a", flush=True)  # Caractère ASCII BEL avec flush pour assurer l'émission immédiate
...
def _run_yt_dlp_for_subtitles(url: str, output_dir: Path) -> tuple[Path | None, str | None]:
    """
    ...
    try:
        ...
        for client in player_clients:
            ...
            logging.info(f"Téléchargement des sous-titres avec yt-dlp (client={client})…")
    ...
    try:
        ...
        for langs in candidates:
            try:
                ...
                text = "\n".join(
                    html.unescape(e.get("text", "")).strip() for e in entries if e.get("text")
                )
                if text.strip():
                    return text
            except Exception:
...
def get_ori_from_youtube(url: str, config: dict, project_name: str) -> bool:
    ...
    try:
        ...
        if not text:
            print("❌ Impossible de récupérer un transcript pour cette URL (yt-dlp et fallback API)")
            return False
        target_path = get_projects_path() / project_name / "ori_vid.txt"
        with open(target_path, "w", encoding="utf-8") as out:
            out.write(text + "\n")
        if orig_lang:
            print(f"✅ Transcript enregistré: {target_path} (langue détectée: {orig_lang})")
        else:
...
def validate_mode_consistency(config, args):
    ...
    if audio_mode in ['openai_tts', 'gemini_tts']:
        ...
        if not config['api']['openai']['api_key'] or config['api']['openai']['api_key'].startswith('sk-'):
            if len(config['api']['openai']['api_key']) < 50:
                logging.error("❌ ERREUR: Clé API OpenAI invalide ou manquante pour le mode TTS")
                logging.error("   → Vérifiez votre clé API dans config.yml")
                sys.exit(1)
    
    elif audio_mode == 'heygen':
        if args.audio:
            logging.error("❌ ERREUR: Option -a (audio TTS) incompatible avec le mode 'heygen'")
            logging.error("   → Changez 'generation_mode: openai_tts' dans config.yml ou utilisez -r au lieu de -a")
            sys.exit(1)
        if args.audio_gemini:
            logging.error("❌ ERREUR: Option -ag (audio Gemini TTS) incompatible avec le mode 'heygen'")
            logging.error("   → Changez 'generation_mode: gemini_tts' dans config.yml ou utilisez -r au lieu de -ag")
            sys.exit(1)
        ...
        logging.error("   → Utilisez 'heygen', 'openai_tts' ou 'gemini_tts' dans config.yml")
        sys.exit(1)
    ...
    if audio_mode == 'openai_tts':
        ...
        try:
            ...
            logging.error("❌ ERREUR: Module 'requests' manquant pour le mode Gemini TTS")
        ...
        try:
            ...
            logging.error("❌ ERREUR: Module 'google-genai' manquant pour le mode Gemini TTS")
...
"""
    ...
    try:
        ...
        messages = [
            {"role": "system", "content": "Vous êtes un expert en génération de keywords pour images d'illustration."},
                    ...
                    with open(csv_path, 'r', encoding='utf-8') as csvfile:
                        ...
                        for row in reader:
                            if row:  # Vérifier que la ligne n'est pas vide
                                existing_keywords.add(row[0])  # Première colonne = keyword
...
"""
    ...
    print("Je suis en train de publier ma vidéo sur youtube. Tu trouveras la miniature de la vidéo dont je me suis inspiré. Génère une miniature similaire pour ma vidéo en français. Crée l'image 16/9 avec la même personne, mais avec un fond en noir et blanc des couleurs jaune ou blanc pour les élements et le texte")
    print("="*80)
    ...
    try:
        ...
        messages = [
            {"role": "system", "content": "Vous êtes un expert en marketing YouTube et en création de contenu viral."},
            {"role": "user", "content": prompt}
        ]
    ...
    try:
        ...
        for rush_id, rush_data in selections.items():
            ...
            for broll in rush_data["brolls"]:
                broll_path = Path("b-roll") / broll["filename"]  # On utilise "filename" au lieu de "path"
...
def main():
    ...
    if args.meta:
        ...
        logging.info(f"Extraction des métadonnées terminée : {success}/{total} vidéos traitées")
        return
    ...
    if args.testbrollchoice:
        ...
        logging.info(f"Test de sélection des b-rolls terminé, résultats sauvegardés dans {json_path}")
        return
    ...
    try:
        ...
        if args.script:
            ...
            try:
                ...
                if result.returncode == 0:
                    logging.info("✅ Script and voice generation completed successfully")
                    logging.info("📋 Output:")
        ...
        if args.audio:
            ...
            try:
                ...
                granularity = config.get('audio', {}).get('openai_tts', {}).get('granularity', 'rush')
                ...
                if granularity == 'bullet':
                    ...
                    for i, bullet_text in enumerate(bullets, 1):
                        ...
                        bullet_text_file = Path(config['paths']['temp']) / f"bullet{i}_text.txt"
                        with open(bullet_text_file, 'w', encoding='utf-8') as f:
                            f.write(bullet_text)
                        
                        # Fichier rush pour compatibilité B-roll existant
                        rush_text_file = Path(config['paths']['temp']) / f"rush{i}_text.txt"
                        with open(rush_text_file, 'w', encoding='utf-8') as f:
                            f.write(bullet_text)
                        
                        logging.info(f"Texte du bullet TTS {i} sauvegardé dans {bullet_text_file} et {rush_text_file}")
                    ...
                    for i, text in enumerate(grouped_paragraphs, 1):
                        ...
                        with open(rush_text_file, 'w', encoding='utf-8') as f:
                            f.write(text)
                        logging.info(f"Texte du rush TTS {i} sauvegardé dans {rush_text_file}")
        ...
        # Étape 2bis: Génération audio TTS (Google Gemini)
            ...
            try:
                ...
                granularity = config.get('audio', {}).get('gemini_tts', {}).get('granularity', 'rush')
                ...
                if granularity == 'bullet':
                    ...
                    for i, bullet_text in enumerate(bullets, 1):
                        ...
                        logging.info(f"Texte du bullet Gemini TTS {i} sauvegardé dans {bullet_text_file} et {rush_text_file}")
                    ...
                    for i, text in enumerate(grouped_paragraphs, 1):
                        ...
                        with open(rush_text_file, 'w', encoding='utf-8') as f:
                            f.write(text)
                        logging.info(f"Texte du rush Gemini TTS {i} sauvegardé dans {rush_text_file}")
