...
using GPT-5 API with JSON-formatted responses.
"""
...
def deprecated(func):
    ...
    def wrapper(*args, **kwargs):
        logging.warning(f"⚠️ DEPRECATED: {func.__name__} is deprecated and will be removed in future versions.")
        return func(*args, **kwargs)
    return wrapper

class Paraphraser:
    """
    Handles text paraphrasing using GPT-5 API.
    ...
    def create_outline_prompt(self, original_text, language="french", target_word_count=None):
        """
        ...
        """
        language_instruction = "Generate the outline in FRENCH language." if language.lower() == "french" else "Generate the outline in ENGLISH language."
        ...
        words_per_key_point = self.config.get('script', {}).get('words_per_keypoint', 80)
        max_keypoints_per_section = self.config.get('script', {}).get('max_keypoints_per_section', 3)
        tone_style = self.config.get('script', {}).get('tone_style', 'pédagogique, rythmé, incisif, conversation orale')
        target_audience = self.config.get('script', {}).get('target_audience', 'investisseurs particuliers, curieux en économie, 30–60 ans')
        ...
        num_key_points = max(3, final_target_words // words_per_key_point)  # Minimum 3 key_points
        
        # Calculate number of sections needed
        num_sections = max(1, (num_key_points + max_keypoints_per_section - 1) // max_keypoints_per_section)
        
        logging.info(f"Word calculation: {target_word_count} → {raw_target_words} → {final_target_words} (capped at 3000)")
        logging.info(f"Key points: {num_key_points} (target: {final_target_words} words)")
        logging.info(f"Sections needed: {num_sections} (max {max_keypoints_per_section} keypoints per section)")

        prompt = [
            {"role": "system", "content": f"""You are an expert in YouTube storytelling and high-retention video scripting.
            ...
            {"role": "user", "content": f"""TRANSCRIPT_SOURCE:
        {original_text}
    ...
    def create_full_script_json(self, outline_data, language="french"):
        ...
        for section_idx, section in enumerate(sections, 1):
            ...
            logging.info(f"📝 Processing section {section_idx}/{total_sections}: {section_title}")
            ...
            for i, key_point in enumerate(key_points, 1):
                logging.info(f"  🔹 Key point {i}/{total_key_points} in section '{section_title}'")
                ...
                if target_words is None:
                    raise ValueError("words_per_keypoint not found in script configuration")
                ...
                for attempt in range(2):  # 2 attempts maximum
                    try:
                        ...
                        response = self.call_gpt5(micro_section_prompt)
                        
                        if isinstance(response, dict) and "paragraph" in response and "title" in response:
                            paragraph = response["paragraph"]
                            title = response["title"]
                            logging.info(f"  ✅ Success on attempt {attempt + 1}")
                            break
                        else:
                            logging.warning(f"  ⚠️ Invalid response format on attempt {attempt + 1}")
                        ...
                        if attempt == 1:  # Last attempt
                            logging.error(f"❌ Failed to generate micro_section {i} after 2 attempts")
                            raise Exception(f"Failed to generate micro_section {i} after 2 attempts: {e}")
                
                if paragraph is None or title is None:
                    raise Exception(f"Failed to generate micro_section {i}: No valid response received")
    ...
    def create_micro_section_prompt(self, key_point_text, target_words, language="french"):
        """
        ...
        writing_style = language_styles.get(language.lower(), language_styles["english"])
...
Generate a short, intelligible voice-over script based on this key point."""}
    ...
    def create_ideas_prompt(self, original_text, language="french"):
        ...
        prompt = [
            {"role": "system", "content": f"""You are an expert content analyst. Transform raw text into structured ideas using markdown format.
...
RESPOND ONLY WITH THE MARKDOWN CONTENT, NO ADDITIONAL TEXT."""},
            ...
            {"role": "user", "content": f"""INPUT TEXT TO STRUCTURE:
{original_text}
...
Convert this text into structured markdown format with clear sections, subsections, and bullet points. Make it easy to understand and follow the logical flow of ideas."""}
    ...
    def call_gpt5(self, messages, max_retries=3):
        """
        Call GPT-5 API with the new responses API.
        ...
        Returns:
            dict: Parsed JSON response from GPT-5
        """
        for attempt in range(max_retries):
            try:
                ...
                logging.error(f"❌ GPT-5 error (attempt {attempt + 1}/{max_retries}): {str(e)}")
    ...
    def call_gpt5_text_only(self, messages, max_retries=3):
        """
        Call GPT-5 API for free-text responses (not JSON).
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Log the response for debugging
                logging.info("=== GPT-5 Text Response ===")
                logging.info(response.output_text)
                ...
                logging.error(f"❌ GPT-5 text error (attempt {attempt + 1}/{max_retries}): {str(e)}")
                if attempt < max_retries - 1:
                    logging.info(f"🔄 Retrying in 2 seconds...")
                    import time
                    time.sleep(2)
    ...
    def verify_paragraphs(self, paragraphs):
        ...
        if len(paragraphs) < min_paragraphs:
            logging.warning(f"Insufficient number of paragraphs: {len(paragraphs)} (minimum: {min_paragraphs})")
            return False
        elif len(paragraphs) > max_paragraphs:
            logging.info(f"✅ Excess paragraphs: {len(paragraphs)} (expected: {expected_paragraphs} ±2)")
            logging.info(f"ℹ️  Pipeline will automatically adapt to {len(paragraphs)} paragraphs - longer video")
        else:
            logging.info(f"✅ Acceptable paragraph count: {len(paragraphs)} (expected: {expected_paragraphs} ±2)")
        ...
        for i, para in enumerate(paragraphs, 1):
            ...
            if words < min_words or words > max_words:
                logging.warning(f"Paragraph {i}: {words} words (out of range {min_words}-{max_words})")
        ...
        if problematic_paragraphs:
            logging.info(f"🔧 Local correction of {len(problematic_paragraphs)} problematic paragraph(s)")
            ...
            if corrected_paragraphs:
                ...
                for i, corrected_para in corrected_paragraphs.items():
                    ...
                    logging.info(f"✅ Paragraph {i+1} corrected: {len(corrected_para.split())} words")
            else:
        ...
        if total_words < min_total or total_words > max_total:
            logging.warning(f"Total: {total_words} words (out of range {min_total}-{max_total})")
            return False
        else:
    ...
    def _fix_short_paragraphs(self, paragraphs, problematic_paragraphs, min_words, max_words):
        ...
        for index, para, current_words in problematic_paragraphs:
            if current_words < min_words:
                ...
                logging.info(f"🔧 Correcting paragraph {index+1}: {current_words} → {target_words} words")
                ...
                messages = [
                    {"role": "system", "content": "You are an expert writer who extends paragraphs naturally and coherently."},
                ...
                try:
                    ...
                    if response and isinstance(response, str):
                        ...
                        if min_words <= corrected_words <= max_words:
                            ...
                            logging.info(f"✅ Paragraph {index+1} corrected: {current_words} → {corrected_words} words")
                        else:
                            logging.warning(f"⚠️ Paragraph {index+1} still incorrect after correction: {corrected_words} words")
                    else:
                        logging.error(f"❌ Invalid response for paragraph {index+1} correction")
                except Exception as e:
                ...
                logging.info(f"🔧 Shortening paragraph {index+1}: {current_words} → {target_words} words")
...
Shortened paragraph:"""
                ...
                messages = [
                    {"role": "system", "content": "You are an expert writer who shortens paragraphs while keeping the essential."},
                ...
                try:
                    ...
                    if response and isinstance(response, str):
                        ...
                        if min_words <= corrected_words <= max_words:
                            corrected_paragraphs[index] = corrected_para
                            logging.info(f"✅ Paragraph {index+1} shortened: {current_words} → {corrected_words} words")
                        else:
                            logging.warning(f"⚠️ Paragraph {index+1} still incorrect after shortening: {corrected_words} words")
                    else:
                        logging.error(f"❌ Invalid response for paragraph {index+1} shortening")
                except Exception as e:
        ...
        for i, para1 in enumerate(normalized_paragraphs):
            for j, para2 in enumerate(normalized_paragraphs[i+1:], i+1):
                ...
                if len(words1) > 0 and len(words2) > 0:
                    ...
                    if similarity > 0.7:
                        logging.error(f"🚨 REPETITION DETECTED between paragraphs {i+1} and {j+1} (similarity: {similarity:.2f})")
    ...
    def process(self, language="french", project=None, input_file_path=None):
        ...
        target_word_count = len(original_text.split())
        logging.info(f"Creating structured ideas from input text... (target: {target_word_count} words)")
        ...
        outline_messages = self.create_outline_prompt(ideas_response, language, target_word_count)
