...
class GeminiTTS:
    ...
    def extract_text_from_full_script_json(self, full_script_path):
        ...
        try:
            ...
            combined_text = "\n\n".join(paragraphs)
            logging.info(
                f"Extracted {len(paragraphs)} paragraphs from full_script.json"
            )
    ...
    def generate_audio(self, text, output_filename):
        ...
        try:
            ...
            payload = {"text": text, "api_key": self.gemini_config["api_key"]}
            ...
            if response.status_code == 200:
                ...
                data = result["candidates"][0]["content"]["parts"][0]["inlineData"][
                    "data"
                ]
                ...
                logging.info(
                    f"🎵 WAV created: {temp_wav_path.name} ({temp_wav_path.stat().st_size} bytes)"
                )
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        logging.info(
                            f"✅ MP3 converted: {output_filename} ({mp3_size} bytes)"
                        )
                        ...
                        speed = self.audio_config.get("voice_settings", {}).get(
                            "speed", 1.0
                        )
                        duration = (words / 150.0) * 60.0 / speed

                        logging.info(
                            f"✅ Audio generated: {output_filename} (~{duration:.2f}s)"
                        )
                        return duration
                    else:
                ...
                error_msg = (
                    f"Gemini proxy error: {response.status_code} - {response.text}"
                )
    ...
    def generate_all_audio(self, script_content):
        ...
        for i, bullet_text in enumerate(bullets, 1):
            ...
            try:
                ...
                logging.info(
                    f"Bullet {i}/{len(bullets)}: {duration:.2f}s (total: {cumulative_time:.2f}s)"
                )
        ...
        logging.info(
            f"✅ All audio generated successfully. Total duration: {cumulative_time:.2f}s"
        )
    ...
    def process(self):
        ...
        try:
            ...
            return {
                "success": True,
                "total_duration": metadata["bullets"][-1]["end_time"]
                if metadata["bullets"]
                else 0,
