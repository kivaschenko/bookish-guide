...
class VectorMatcher:
    ...
    def __init__(
        ...
        config=None,
        broll_folder="b-roll",
        embeddings_file="broll_vector_embeddings.json",
        ...
        # Récupérer la clé API et le modèle depuis la config
        if (
            config
            and "api" in config
            ...
            raise ValueError(
                "vector_matcher.py: Configuration OpenAI manquante dans config.yml (api.openai.api_key et api.openai.model)"
            )
    ...
    def load_broll_descriptions(self):
        ...
        for filepath in glob.glob(os.path.join(self.broll_folder, "*.txt")):
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                if video_file:
                    brolls.append({"filename": video_file, "description": description})
                else:
                    # Fallback: utiliser .mp4 par défaut (comportement original)
                    brolls.append(
                        {"filename": base_name + ".mp4", "description": description}
                    )
        return brolls
    ...
    def compute_or_load_embeddings(self):
        ...
        """
        embeddings_path = os.path.join(self.broll_folder, self.embeddings_file)
...
{{
  ...
  "phrase_3": [{{"genre": "homme", "age": 28}}]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    with open(
                        "temp/character_analysis.json", "w", encoding="utf-8"
                    ) as f:
                        json.dump(character_analysis, f, ensure_ascii=False, indent=2)

                    print(
                        f"✅ Analyse des personnages sauvegardée dans temp/character_analysis.json"
                    )
                    return character_analysis

                except json.JSONDecodeError as e:
                    print(
                        f"❌ Erreur de parsing JSON de l'analyse des personnages: {e}"
                    )
                ...
                print(
                    f"Erreur API OpenAI pour l'analyse des personnages: {response.status_code}"
                )
                return {}
    ...
    def ask_gpt_to_choose_best_broll(
        self,
        sentence,
...
Réponds UNIQUEMENT les 3 numéros des meilleures vidéos, séparés par des virgules (ex: 2,5,8)."""
        ...
        with open(
            "log-openai-ask_gpt_to_choose_best_broll.txt", "a", encoding="utf-8"
        ) as f:
            f.write(f"\n--- Phrase: {sentence} ---\n{prompt}\n")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    print(
                        f"GPT a retourné un format invalide: {content}, utilisation des 3 premiers choix par défaut"
                    )
                    return [0, 1, 2]
            else:
    ...
    def _log_to_html(self, prompt, response, broll_filename):
        ...
        image_tag = (
            f'<img src="{image_path}" style="max-width:300px; max-height:200px; border-radius:8px;">'
            if image_path
    ...
    def select_brolls_for_rush(self, paragraph_text, used_brolls_global_set=None):
        ...
        for i, sentence in enumerate(sentences):
            ...
            available_brolls = [
                ...
                if b["filename"] not in used_brolls_global_set
                and b["filename"] not in used_brolls_local
            ]
            ...
            similarities = cosine_similarity(np.array([query_vec]), broll_embeddings)[0]
            # Top 10
            top_indices = similarities.argsort()[-10:][::-1]
            top_brolls_with_scores = [
                (available_brolls[idx], similarities[idx]) for idx in top_indices
            ]
            ...
            if i > 1:  # Au moins deux phrases précédentes
                previous_sentences.insert(
                    0, sentences[i - 2]
                )  # Insérer au début pour garder l'ordre chronologique
            ...
            for j, broll in enumerate(chosen_brolls):
                if j == 0:
                    print(
                        f"✅ GPT-4o a choisi (broll) : {broll['filename']} (option {gpt_choice_indices[j] + 1})"
                    )
                    print(
                        f"📄 Description : {broll['description'][:150]}{'...' if len(broll['description']) > 150 else ''}"
                    )
                elif j == 1:
                    print(
                        f"🔄 B-roll 2 : {broll['filename']} (option {gpt_choice_indices[j] + 1})"
                    )
                    print(
                        f"📄 Description broll2 : {broll['description'][:150]}{'...' if len(broll['description']) > 150 else ''}"
                    )
                elif j == 2:
                    print(
                        f"🔄 B-roll 3 : {broll['filename']} (option {gpt_choice_indices[j] + 1})"
                    )
                    print(
                        f"📄 Description broll3 : {broll['description'][:150]}{'...' if len(broll['description']) > 150 else ''}"
                    )

            # Stocker les 3 b-rolls dans le JSON
            selections[str(i + 1)] = {
                "broll": chosen_brolls[0]["filename"]
                if len(chosen_brolls) > 0
                else None,
                "broll2": chosen_brolls[1]["filename"]
                if len(chosen_brolls) > 1
                else None,
                "broll3": chosen_brolls[2]["filename"]
                if len(chosen_brolls) > 2
                else None,
            }
