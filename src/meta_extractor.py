...
class MetaExtractor:
    ...
    def __init__(self, config):
        ...
        """
        self.config = config
        self.b_roll_path = "b-roll"  # Directory containing b-roll videos
        ...
        if not self.api_key:
            ...
            if self.api_key:
                ...
                logging.warning("OpenAI API key not found in configuration or environment variables. "
    ...
    def extract_first_frame(self, video_path):
        ...
        try:
            ...
            if os.path.exists(jpg_path):
                logging.info(f"Image already extracted for {video_name}, using existing one")
                return jpg_path
    ...
    def analyze_image(self, image_path):
        ...
        try:
            ...
            if not os.path.exists(image_path):
                return {"error": f"L'image {os.path.basename(image_path)} n'existe pas", "description": "", "context": ""}
            
            # Vérifier la clé API
            if not self.api_key:
                return {"error": "Clé API OpenAI manquante", "description": "", "context": ""}
            
            # Préparation de l'image pour l'API
            with open(image_path, "rb") as image_file:
                image_data = image_file.read()
            
            logging.info(f"Sending image {os.path.basename(image_path)} to OpenAI API for analysis...")
            ...
            # Construction de la requête pour GPT-5 avec Vision
            payload = {
                "model": "gpt-5",
                "messages": [
                    {
...
Mots-clés : médecine, hôpital, infirmière, années 1950, diagnostic, travail, solitude, tension, vintage, maladie"""
            ...
            if response.status_code != 200:
                logging.error(f"Erreur API OpenAI: {response.status_code} - {response.text}")
                return {"error": f"Erreur API: {response.status_code}", "description": "", "context": ""}
            
            # Traitement de la réponse
            response_content = response.json()
            if 'choices' in response_content:
                # Extraire directement le texte de la réponse
                analysis_text = response_content['choices'][0]['message']['content'].strip()
            ...
            logging.error(f"Erreur lors de l'analyse de l'image {os.path.basename(image_path)}: {e}")
            traceback.print_exc()
    ...
    def process_video(self, video_path):
        ...
        try:
            ...
            if not jpg_path:
                ...
                with open(txt_path, 'w', encoding='utf-8') as f:
                    f.write(f"Erreur: Impossible d'extraire la première frame de {video_name}")
                return False
            ...
            logging.error(f"Erreur lors du traitement de {os.path.basename(video_path)}: {e}")
            traceback.print_exc()
            return False
    ...
    def process_all_videos(self):
        ...
        videos = [os.path.join(self.b_roll_path, f) for f in os.listdir(self.b_roll_path) 
        ...
        # Commentez ou supprimez ces 3 lignes pour traiter toutes les vidéos
        # videos = videos[:1]
        # logging.info(f"MODE DEBUG: Traitement limité à la première vidéo uniquement")
        # total = 1
        ...
        for i, video_path in enumerate(videos):
            logging.info(f"[{i+1}/{total}] Traitement de {os.path.basename(video_path)}...")
